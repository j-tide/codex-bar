...
日期 tag 是对外版本号，例如 `v2026.06.18` / `v2026.06.18.1`。脚本会把日期写入 `CFBundleShortVersionString`（`2026.06.18`），并把完整版本写入 `CFBundleVersion`（`20260618` / `20260618.1`）；App 会显示完整的日期版本，并用它判断自动更新。发布包统一命名为 `codexAppBar-vYYYY.MM.DD[.N]-release.zip` 和 `codexAppBar-vYYYY.MM.DD[.N]-setup.dmg`。DMG 内含 Applications 快捷方式，可拖拽安装；两种产物均上传并校验 SHA-256。
